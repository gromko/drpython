...
import os, sys, atexit, psutil, shutil, re, string, traceback, logging
import wx, wx.stc
...
import importlib
import subprocess
from drSplash import drSplashScreen
from drModeDialog import drMode
from drSyntaxCheck import drSyntaxChecker
...
ukr_popupmenulist = {"Скасувати": "Undo",
"Повернути": "Redo",
"<Separator>":"<Separator>",
"Вирізати": "Cut",
"Копіювати":"Copy",
...
def check_python_installed():
    try:
        # Спробуємо виконати команду python --version
    ...
    try:
        # Спробуємо виконати команду python3 --version
        result = subprocess.run(['python3', '--version'], capture_output=True, text=True)
        if result.returncode == 0:            
            return True
    except FileNotFoundError:
        pass
    wx.MessageBox("У системі не знайдено встановленого інтерпретатора Python.\nВи зможете завантажити Python з сайту www.python.org/downloads та встановити його", "Не знайдено Python", wx.ICON_EXCLAMATION)
...
class DrTaskBarIcon(TaskBarIcon):
    def __init__(self, frame,icon):
        TaskBarIcon.__init__(self)       

        self.SetIcon(icon, 'drPython - програмувати просто!')

#----------------------------
class PyDocViewer(wx.Frame):
    def __init__(self, parent, title="PyDoc Viewer"):
        super(PyDocViewer, self).__init__(parent, title=title, size=(800, 600))
        self.html_window = wx.html.HtmlWindow(self)
        self.CreateUI()

    def CreateUI(self):
        """Створення інтерфейсу для відображення документації."""
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.Add(self.html_window, 1, wx.EXPAND)
        self.SetSizer(vbox)

    def ShowDocumentation(self, html_content):
        """Відображає HTML-документацію у вікні."""
...
class DrFrame(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        wx.Frame.__init__(self, parent, id, title, wx.DefaultPosition, (1024, 768))
        #self.ignoreevents = False

        self.disableeventhandling = False
        self.firstrun = False
        self.idemode = True
        self.defaultidemode = "Класичний"
        ...
        self.ddirectory = os.path.expanduser("~")+"/drPythonProjects" #projects directory        
        if not os.path.exists(self.ddirectory):
            os.makedirs(self.ddirectory)
        ...
        self.firstrun = not os.path.exists(self.prefsfile)
        ...
        icon = wx.Icon(self.bitmapdirectory+iconname, wx.BITMAP_TYPE_PNG)
        self.SetIcon(icon)       
        self.tb_icon = DrTaskBarIcon(self,icon)
        ...
        if self.prefs.rememberwindowsizeandposition:
            ...
            if os.path.exists(sizeandpositionfile):
                try:
                    ...
                    self.ShowMessage("Помилка завантаження розміру вікна.  Файл '%s' можливо є пошкодженим." % sizeandpositionfile, "Помилка")
        ...
        self.documentnotebook.AddPage(drPanel(self.documentnotebook, self.ID_APP), "Без назви 1")        
        self.promptnotebook.AddPage(drPanel(self.promptnotebook, self.ID_APP), "Консоль") #add prompt notebook page
        ...
        try:
            ...
            self.ShowMessage("Помилка під час встановлення поточного каталогу: '%s'" % self.ddirectory, "Помилка налаштувань")
            self.ddirectory = self.programdirectory
            os.chdir(self.ddirectory)
        ...
        #
        imagedir = os.path.join(self.programdirectory, "image")
        # Створення панелі для "статусбару" з кнопками
        self.tool_panel = wx.Panel(self)
               
        # Завантаження зображення з файлу
        logo_img = wx.Image(imagedir+"/logo.png", wx.BITMAP_TYPE_ANY)

        # Створення елементу StaticBitmap для відображення зображення
        self.logo = wx.StaticBitmap(self.tool_panel, -1, wx.Bitmap(logo_img))
        
        # Підказки
        self.tool_msg = wx.StaticText(self.tool_panel,-1,style = wx.ALIGN_LEFT)
        self.tool_msg.SetLabel("")
        self.tips = DrTips(self,self.tool_msg)
        
        # Панель для кнопок        
        tool_sizer = wx.BoxSizer(wx.HORIZONTAL)        
        # Створення кнопок
        run = wx.Bitmap(imagedir+"/play.png", wx.BITMAP_TYPE_ANY)
        self.runBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=run, size=(48, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.runBtn.Bind(wx.EVT_BUTTON, self.OnRun)
        
        stop = wx.Bitmap(imagedir+"/stop.png", wx.BITMAP_TYPE_ANY)
        self.stopBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=stop, size=(48, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.stopBtn.Bind(wx.EVT_BUTTON, self.OnEnd)
        
        ttools = wx.Bitmap(imagedir+"/ttools.png", wx.BITMAP_TYPE_ANY)
        self.ttoolsBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=ttools, size=(24, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.ttoolsBtn.Bind(wx.EVT_BUTTON, self.OnShowHide)
        
        console = wx.Bitmap(imagedir+"/console.png", wx.BITMAP_TYPE_ANY)
        self.consoleBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=console, size=(48, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.consoleBtn.Bind(wx.EVT_BUTTON, self.OnTogglePrompt)
        
        newf = wx.Bitmap(imagedir+"/new.png", wx.BITMAP_TYPE_ANY)
        self.newBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=newf, size=(48, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.newBtn.Bind(wx.EVT_BUTTON, self.OnNew)
        
        openf = wx.Bitmap(imagedir+"/open.png", wx.BITMAP_TYPE_ANY)
        self.openBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=openf, size=(48, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.openBtn.Bind(wx.EVT_BUTTON, self.OnOpen)
        
        savef = wx.Bitmap(imagedir+"/save.png", wx.BITMAP_TYPE_ANY)
        self.saveBtn = wx.BitmapButton(self.tool_panel, id=wx.ID_ANY, bitmap=savef, size=(48, 48),style=wx.NO_BORDER|wx.BU_EXACTFIT)
        self.saveBtn.Bind(wx.EVT_BUTTON, self.OnSaveAs)
    
        tool_sizer = wx.BoxSizer(wx.HORIZONTAL)
        tool_sizer.Add(self.logo, 0, wx.ALL, 5)
        tool_sizer.Add(self.tool_msg, 0, wx.ALL, 5)
        tool_sizer.AddStretchSpacer()  # Додаємо "розтягнутий" простір
        tool_sizer.Add(self.runBtn, 0, wx.ALL, 5)
        tool_sizer.Add(self.stopBtn, 0, wx.ALL, 5)
        tool_sizer.Add(self.ttoolsBtn, 0, wx.ALL, 5)
        tool_sizer.Add(self.consoleBtn, 0, wx.ALL, 5)
        tool_sizer.Add(self.newBtn, 0, wx.ALL, 5)    
        tool_sizer.Add(self.openBtn, 0, wx.ALL, 5)
        tool_sizer.Add(self.saveBtn, 0, wx.ALL, 5)
        self.tool_panel.SetSizer(tool_sizer)

        # Приховуємо кнопки Open та Save за замовчуванням
        self.stopBtn.Hide()
        self.consoleBtn.Hide()
        self.newBtn.Hide()  
        self.openBtn.Hide()
        self.saveBtn.Hide()
        ...
        self.bSizer.Add(self.tool_panel, 0, wx.EXPAND | wx.ALL, 5) #add main panel
        ...
        self.Bind(wx.EVT_TOOL, self.OnDayNight, id=self.ID_DAY_NIGHT)
        ...
        for f in sys.argv[1:]:
            if not os.path.exists(f):
                if self.Ask("'" + f + "' не існує.  Створити?", "Файл не існує"):
                    try:
                        fh = open(f, 'w', encoding='utf-8')
                        fh.close()
                    except:
                        self.ShowMessage("Помилка при створенні '" + f + "'")
                        continue
                else:
                  continue
        ...
        if self.firstrun:
            mode = drMode(self)            
            self.firstrun = False
            if self.idemode == False:
                self.prefs.defaultidemode = "ЄPython"
                self.defaultidemode = "ЄPython"
            else:
                self.prefs.defaultidemode = "Класичний"
                self.defaultidemode = "Класичний"
            self.SavePrefs()
               
        # Створюємо та показуємо splash screen
        if self.prefs.showsplash == 1:
            # Шлях до PNG зображення заставки
            imagepath = imagedir+"/splash.png" 
            splash = drSplashScreen(imagepath)
        
        # ----------------------------------------
        
        if self.prefs.defaultidemode == "ЄPython":
            self.ToolBarToggle()
 
    def SavePrefs(self):        
        self.WriteUserPreferencesDirectoryFile()
        if not os.path.exists(self.preferencesdirectory):
            drScrolledMessageDialog.ShowMessage(self, "Тека для збереження налаштувань '" + self.preferencesdirectory + "' не існує.", "Помилка drPython")
            return
        preffile = self.prefsfile
        try:
            drPrefsFile.WritePreferences(self.prefs, preffile)
        except IOError:
            drScrolledMessageDialog.ShowMessage(self, "Виникли проблеми при збереженні до:\n"  + preffile, "Помилка запису")
            return

    def ThemeChange(self):
        """Налаштування стилів для Python з урахуванням теми."""
        ...
        if self.is_dark_theme:
            # Темна тема
            self.txtDocument.StyleSetSpec(stc.STC_P_DEFAULT, "fore:#FFFFFF,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_COMMENTLINE, "fore:#7F7F7F,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_COMMENTBLOCK, "fore:#0B5400,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_STRING, "fore:#00FF00,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_CHARACTER, "fore:#00FF00,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_WORD, "fore:#FF0000,back:#000000,size:12,face:%s" %fontface )  # Ключові слова
            self.txtDocument.StyleSetSpec(stc.STC_P_IDENTIFIER, "fore:#FFFFFF,back:#000000,size:12,face:%s" %fontface )  # Ідентифікатори
            self.txtDocument.StyleSetSpec(stc.STC_P_TRIPLE, "fore:#7B8184,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_TRIPLEDOUBLE, "fore:#7B8184,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_CLASSNAME, "fore:#FF00FF,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_DEFNAME, "fore:#FF4500,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_OPERATOR, "fore:#FF0071,back:#000000,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_NUMBER, "fore:#00BFFF,back:#000000,size:12,face:%s" %fontface )
        else:
            # Світла тема
            self.txtDocument.StyleSetSpec(stc.STC_P_DEFAULT, "fore:#000000,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_COMMENTLINE, "fore:#7F7F7F,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_COMMENTBLOCK, "fore:#0B5400,back:#BAE8D5,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_STRING, "fore:#007f08,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_CHARACTER, "fore:#007f08,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_WORD, "fore:#FF0000,back:#FFFFFF,size:12,face:%s" %fontface )  # Ключові слова
            self.txtDocument.StyleSetSpec(stc.STC_P_IDENTIFIER, "fore:#000000,back:#FFFFFF,size:12,face:%s" %fontface )  # Ідентифікатори
            self.txtDocument.StyleSetSpec(stc.STC_P_TRIPLE, "fore:#7B8184,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_TRIPLEDOUBLE, "fore:#7B8184,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_CLASSNAME, "fore:#b200ac,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_DEFNAME, "fore:#e80300,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_OPERATOR, "fore:#FF0071,back:#FFFFFF,size:12,face:%s" %fontface )
            self.txtDocument.StyleSetSpec(stc.STC_P_NUMBER, "fore:#0027c4,back:#FFFFFF,size:12,face:%s" %fontface )

             
    def ToggleShowHide(self):
        # Перемикаємо видимість кнопок Open та Save
            if self.openBtn.IsShown():
                self.consoleBtn.Hide()
                self.newBtn.Hide()
                self.openBtn.Hide()
                self.saveBtn.Hide()
            else:
                self.consoleBtn.Show()
                self.newBtn.Show()
                self.openBtn.Show()
                self.saveBtn.Show()
            self.tool_panel.Layout()              
    ...
    def Append_Menu(self, menu, id, s, LaunchesDialog = False, AmpersandAt = -1, absolutelabel=""):
        ...
        if label in ukr_popupmenulist:
            s_img = ukr_popupmenulist[label]
        else:
            s_img = label  
        item = wx.MenuItem(menu, id, label, label) 
        menuiconfile = os.path.join(self.bitmapdirectory, "16", s_img + ".png")
    ...
    def AskExitingDrPython(self):
        if self.Ask("Бажаєте закінчити роботу з drPython?"):
            sys.exit()
    ...
    def CheckSyntax(self, docnumber=-1):
        ...
        if not self.txtDocumentArray[docnumber].filename:
            self.ShowMessage("Не можу перевірити файл '%s'" % fn, "Неможливо перевірити неназваний файл")
            return False
        #Check Syntax First
        check =  drSyntaxChecker(fn)
        e,l = check.getResult()
        if e:
            self.setDocumentTo(docnumber)
            self.txtDocument.ScrollToLine(l-1)
            self.txtDocument.GotoLine(l-1)
    ...
    def GetAlreadyOpen(self):
        ...
        return list(map(_get_filename, self.txtDocumentArray))
    ...
    def GetNewId(self):
        return int(wx.Window.NewControlId(count=1)) # wx.NewId()
    ...
    def GetPluginLabels(self, filename, doNotAppend = False):
        try:
            ...
            self.ShowMessage("Помилка файлу: '" + filename + "'.", "Помилка")
            return []
    ...
    def InitializeConstants(self):
        ...
        if not self.preferencesdirectory:
            ...
            self.preferencesdirectoryprefix = ".drpython-v"+DRPY_VER
            if self.PLATFORM_IS_WIN:
                self.preferencesdirectoryprefix = "drpython-v"+DRPY_VER
    ...
    def InitializePlugin(self, plugin, ShowDialog = True):
        ...
        if plugin in self.LoadedPlugins:
            if ShowDialog:
                self.ShowMessage("'" + plugin + "' вже завантажено.\ndrPython не перезавантажує це доповнення.", "Доповнення вже завантажено")
            return
        ...
        try:
            ...
            self.ShowMessage("Помилка завантаження: " + plugin + "\ndrPython не може завантажити це доповнення.", "Помилка доповнення")
    ...
    def LoadPlugins(self, plugins_file = ""):
        ...
        if os.path.exists(pluginsfile):
            try:
                ...
                self.ShowMessage(("Помилка завантаження: " + pluginsfile + "\nDrPython не може завантажити це доповнення."), "Помилка доповнення")
        else:
            try:
                ...
                self.ShowMessage("Помилка створення індексу доповнень.\n\nДоповнення можуть бути невідповідними.", "Помилка доповнень")
    ...
    def LoadPluginShortcuts(self, plugin):
        ...
        try:
            ...
            self.ShowMessage(("Помилка завантаження: " + plugin + "\ndrPython не може завантажити сполучення клавіш для цього доповнення."), "Помилка сполучень клавіш доповнення")
    ...
    def LoadPopUpFile(self):
        ...
        if os.path.exists(popupfile):
            try:
                ...
                self.ShowMessage("Помилка завантаження: " + popupfile + "\nDrPython буде використовуати системні налаштування.", "Помилка вигулькного меню")
    ...
    def SetDrPythonDirectories(self):
        ...
        if not os.path.exists(self.bitmapdirectory):
            self.ShowMessage("Тека зображень (" + self.bitmapdirectory + ") не існує.", "Критична помилка drPython")
            sys.exit(1)
    ...
    def LoadPreferences(self):
        ...
        if os.path.exists(self.prefsfile):
            try:
                ...
                self.ShowMessage(("Помилка завантаження: " + self.prefsfile + "\nDrPython буде використовувати налаштування за замовчуванням."), "Помилка налаштувань")
                self.prefs.reset()
    ...
    def LoadRecentFiles(self):
        ...
        if not os.path.exists(filename):
            try:
                ...
                if self.prefs.debugmodus:
                    self.ShowMessage(
                        "Помилка при створенні: %r\n"
                        "Можливо у вас немає прав доступу або місце збереження є переповненим" % filename,
                        "Помилка списку нещодавніх файлів"
                    )
        ...
        try:
            ...
            self.ShowMessage("Помилка зчитування: " + filename, "Помилка списку нещодавніх файлів")
            self.AskExitingDrPython()
    ...
    def OnCheckIndentation(self, event):
        ...
        if result == 2:
            msg = "У цьому документі не знайдено відступів."
        elif result == -1:
            msg = "У цьому документі для відступів використовуються пропуски(spaces)."
        elif result == 1:
            msg = "У цьому документі для відступів використовуються табуляції(tabs)."
        elif result == 0:
            msg = "У цьому документі для відступів використовуються пропуски(spaces) та табуляції(tabs)."
        wx.EndBusyCursor()
        self.ShowMessage(msg, "Перевірка типу відступів у коді")

    def OnCheckSyntax(self, event):
        self.CheckSyntax(self.docPosition)
        #self.prefs.enablefeedback:
    ...
    def OnClose(self, event):
        ...
        if self.txtDocument.GetModify():
            ...
            answer = wx.MessageBox("Бажаєте зберегти файл '%s'?" % self.txtDocument.GetFilename(), "drPython", wx.YES_NO | wx.CANCEL | wx.ICON_QUESTION)
            #end limodou
            if answer == wx.YES:
    ...
    def OnDayNight(self, event):        
        ...
        self.ThemeChange()
        if self.is_dark_theme == False:
            # Світла тема
            self.txtDocument.StyleSetBackground(stc.STC_STYLE_DEFAULT, wx.Colour(255, 255, 255))
            self.txtDocument.StyleSetForeground(stc.STC_STYLE_DEFAULT, wx.Colour(0, 0, 0))
            self.txtDocument.SetCaretLineBackground('#a9ffff')
            self.txtDocument.SetCaretForeground('#ff0000')
                            
        else:
            # Темна тема
            self.txtDocument.StyleSetBackground(stc.STC_STYLE_DEFAULT, wx.Colour(0, 0, 0))
            self.txtDocument.StyleSetForeground(stc.STC_STYLE_DEFAULT, wx.Colour(255, 255, 255))
            self.txtDocument.SetCaretLineBackground('#011d50')
            self.txtDocument.SetCaretForeground('#ffff00') 
          
        # Оновлення редактора
        self.txtDocument.Refresh()
    ...
    def OnViewPyDoc(self, event):
        """Відображає документацію в окремому вікні."""
        self.promptSaveAll()

        # Додаємо шляхи до всіх відкритих файлів до sys.path
        for x in range(len(self.txtDocumentArray)):
            filePath = self.getFilePath(self.txtDocumentArray[x].GetFilename())
            sys.path.append(filePath[0])

        # Генеруємо HTML-документацію для поточного файлу
        if self.txtDocument:
            filePath = self.getFilePath(self.txtDocument.GetFilename())
            module_name = os.path.splitext(filePath[1])[0]  # Видаляємо розширення .py
            try:
                module = __import__(module_name)
                html_content = pydoc.HTMLDoc().document(module)
            except ImportError:
                wx.MessageBox(f"Failed to import module: {module_name}", "Error", wx.ICON_ERROR)
                return

            # Відображаємо документацію в окремому вікні
            viewer = PyDocViewer(self, title=f"PyDoc: {module_name}")
            viewer.ShowDocumentation(html_content)
        else:
            wx.MessageBox("No current file to generate documentation!", "Error", wx.ICON_INFORMATION)
    ...
    def OnCloseAllDocuments(self, event):
        ...
        while x > -1:
            ...
            if self.txtDocument.GetModify():
                #prompt saving filename limodou 2004/04/19
                if self.Ask("Бажаєте зберегти '%s'?" % self.txtDocument.GetFilename(), "drPython"):
                #end limodou
                    self.OnSave(event)
            self.OnClose(event)
            x = x - 1
    ...
    def OnCloseAllOtherDocuments(self, event):
        if not self.txtDocument.filename:
            self.ShowMessage("Sorry, does not work when an untitled file is selected.", "Помилка drPython")
            return
        farray = list(map(lambda document: document.filename, self.txtDocumentArray))
        try:
            i = farray.index(self.txtDocument.filename)
        except:
            #franz: (Updated Namespace)
            self.ShowMessage("Something went wrong trying to close all other tabs.", "Помилка drPython")
            return
    ...
    def OnClosePrompt(self, event):
        ...
        if len(self.txtPromptArray) > 1:
            ...
            self.UpdateMenuAndToolbar()
            self.promptnotebook.SetPageText(self.promptPosition, "Консоль")
            #The set size stuff ensures that wx.widgets repaints the tab.
            x, y = self.GetSize()
            self.SetSize((x-1, y-1))
            self.SetSize((x, y))
            ...
            try:
                ...
                while x < l:
                    if self.txtDocumentArray[x].GetModify():
                        answer = wx.MessageBox("Бажаєте зберегти файл '%s'?" % self.txtDocumentArray[x].GetFilename(),
                ...
                if self.prefs.alwayspromptonexit:
                    if not self.Ask("Бажаєте закінчити роботу та вийти з програми?   ", "drPython"):
                        return
    ...
    def OnEditBookmarks(self, event):
        ...
        print("drBookmarksDialog imported")
        d = drBookmarksDialog(self, os.path.join(self.datdirectory, "bookmarks.dat"))
        print("drBookmarksDialog creted from file")
        d.ShowWindowModal()
    ...
    def OnRunScript(self, lookscript):
        ...
        if not lookscript in self.drscriptmenu.scripts:
            wx.MessageBox("'" + lookscript + "' не знайдено", "Run DrScript", wx.ICON_EXCLAMATION)
        else:
    ...
    def OnRunPlugin(self, lookplugin):
        if lookplugin in self.PluginShortcutFunctionNames:
            ...
            wx.MessageDialog(self, "'" + lookplugin + "' не знайдено", "Run PluginFunction", wx.ICON_EXCLAMATION).ShowModal()

    def updateUIisEnd(self):
        self.run_state = False
        self.txtPrompt.SetReadOnly(True)
        self.SetStatusText("", 2)
        self.toolbar.EnableTool(self.ID_END, False)
        if self.stopBtn.IsShown():
            self.runBtn.Show()
            self.stopBtn.Hide()
            self.tool_msg.SetLabel("Виконання завершено")
            self.tool_panel.Layout()
        self.UpdateMenuAndToolbar() 
    ...
    def Terminate(self):
        if self.run_state:
            ...
            # Додаємо повідомлення у консоль перед завершенням
            self.txtPrompt.AddText("\n*** [exec stopped by user]\n")
            # Завершуємо процес
            self.txtPrompt.TerminateScript()
            self.updateUIisEnd()
    ...
    def OnEndX(self, event):
        if self.PLATFORM_IS_WIN:     
                ...
                if self.stopBtn.IsShown():
                    self.runBtn.Show()
                    self.stopBtn.Hide()
                    self.tool_msg.SetLabel("Виконання зупинено")
                    self.tool_panel.Layout() 
        else:
            self.Terminate()
    ...
    def OnFindAndComplete(self, event):        
        '''
        Пошук варіантів для автоматичного доповнення
        '''
        #Список стандартних ключових слів Python
        python_keywords = [
        ...
        # Отримання лівої частини тексту
        ...
        # Додавання ключових слів Python до результатів
        for keyword in python_keywords:
            if keyword.startswith(oword) and keyword not in r:
                r.append(keyword)
    ...
    def OnGoTo(self, event):
        d = wx.TextEntryDialog(self, "Перейти до рядка за номером:", "drPython - перейти", "")
        answer = d.ShowModal()
        v = d.GetValue()
        d.Destroy()
        if answer == wx.ID_OK:
            try:
                ...
                if (v >= 0) and (v < self.txtDocument.GetLineCount()):
                    ...
                    self.ShowMessage("Такого рядка не існує", "Помилка drPython")
            except StandardError:
                self.ShowMessage("Ви маєте ввести ціле число (1, 2, 3, і т.п.)", "Помилка drPython")
    ...
    def OnInsertRegEx(self, event):
        ...
        d = drRegularExpressionDialog(self, -1, "Вставити регулярний вираз", self.txtPrompt.GetSTCFocus())
        d.Show()

    def OnInsertSeparator(self, event):
        d = drSeparatorDialog(self, "Вставити розділювач в код")
        answer = d.ShowModal()
        label,bg_color = d.GetLabel()
        label = '# --- '+label+' ------------------------------------'
        d.Destroy()
        if answer == wx.ID_OK:
            pos = self.txtDocument.GetCurrentPos()            
            ...
            self.ApplyLineStyle(self.txtDocument.GetCurrentLine(), bg_color)
            



    def ApplyLineStyle(self, line_number, bg_color):
                
        # Додаємо маркер для виділення рядка
        self.marker_num = 1
        self.txtDocument.MarkerDefine(self.marker_num, stc.STC_MARK_BACKGROUND)
        self.txtDocument.MarkerSetBackground(self.marker_num,  bg_color)  # Колір фону


        # Виділяємо рядок
        self.txtDocument.MarkerAdd(line_number, self.marker_num)


            
    def ToolBarToggle(self):
            # Перемикання видимості панелі інструментів
            self.consoleBtn.Hide()
            self.newBtn.Hide()
            self.openBtn.Hide()
            self.saveBtn.Hide()
            self.tool_panel.SetSize((100,50)) 
            x,y = self.tool_panel.GetPosition()
            pw,ph = self.GetSize()
            w, h = self.tool_panel.GetSize()
            if x>(pw/3):
               self.tool_panel.SetPosition((x+250,y))  
            if self.toolbar.IsShown():
                self.toolbar.Show(False)
                self.tool_panel.Show(True)
            else:
                self.toolbar.Show(True)
                self.tool_panel.Show(False)                
            self.toolbar.Parent.Layout()
    ...
    def OnMenuFind(self, event):
        ...
        d = drFindReplaceDialog(self, -1, "Пошук", stc)
    ...
    def OnMenuReplace(self, event):
        stc = self.GetActiveSTC()
        d = drFindReplaceDialog(self, -1, "Заміна", stc, 1)
    ...
    def OnNew(self, event):
        ...
        self.documentnotebook.AddPage(nextpage, "Без назви " + str(last))
    ...
    def OnNewPrompt(self, event):
        ...
        self.promptnotebook.AddPage(nextpage, "Консоль")
    ...
    def OnOpen(self, event):
        dlg = drFileDialog.FileDialog(self, "Відкрити", self.prefs.wildcard, MultipleSelection=True, ShowRecentFiles=True)
        if self.ddirectory:
            try:
                dlg.SetDirectory(self.ddirectory)
            except:
                #self.ShowMessage("Помилка налаштування теки:" + self.ddirectory, "Помилка drPython")
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for fname in filenames:
                if not os.path.exists(fname):
                    if wx.MessageBox("'" + fname + "' не існує. Створити його?",
                        "Попередження", wx.YES_NO | wx.NO_DEFAULT | wx.ICON_QUESTION) == wx.YES:
    ...
    def OnPrefs(self, event):
        ...
        d = drPrefsDialog(self, -1, "drPython - Налаштування")
        d.ShowModal()
        d.Destroy()
    ...
    def OnProcessEnded(self, event):        
        ...
        epid = event.GetPid()
        try:
            i = list(map(lambda tprompt: tprompt.pid == epid, self.txtPromptArray).index(True))
        except:
            return
    ...
    def OnRestoreFromBackup(self, event):
        if os.path.exists(self.txtDocument.filename + ".bak"):
            ...
            self.ShowMessage("No Backup File For: \"" + self.txtDocument.filename + "\"", "Помилка drPython")

    def OnRun(self, event):        
        if self.run_state:                
                self.OnEnd(event) # зупиняємо виконання, якщо працює програма користувача                
                return                
        self.toolbar.EnableTool(self.ID_END,True)
     
        """ Якщ документ було змінено - зберігаємо """
        if self.txtDocument.GetModify():
            if self.prefs.saveonrun or self.Ask("Файл був змінений " + "і має бути збережений, перш ніж його можна буде запустити.\nЗберегти його?", "drPython"):
                if not self.OnSave(event):
                    return
            else:
                return
        ...
        if self.checkiffileisPython (self.txtDocument.filename):
            ...
            if self.PLATFORM_IS_WIN:
                self.runcommand((self.pythexecw +
                                " -u " +
                                self.prefs.pythonargs +
                                " " +
                                filen +
                                " " +
                                largs),
                                "Виконується " + filen, filen)
            else:
                self.runcommand(("python3"+ ' ' + self.txtDocument.filename + ' '  + largs), "Виконується " + filen, filen)                #patch: [ 1366679 ] Goto Line Should Not Display At Top Of Window
        elif ext == ".lua":
    ...
    def OnSaveAll(self, event):
        ...
        if self.prefs.promptonsaveall:
            ...
            for document in self.txtDocumentArray:
                ...
                x += 1
            if not tosaveLabels:
                self.ShowMessage("Немає змінених файлів.", "Зберегти все")
                return
            d = wx.lib.dialogs.MultipleChoiceDialog(self, "Зберегти всі змінені файли?", "Зберегти все", tosaveLabels, size=(300, 300))
            l = len(tosaveArray)
            y = 0
            while y < l:
    ...
    def OnSaveAs(self, event):
        # якщо файл ще не має імені
        if not self.txtDocument.filename or self.txtDocument.filename.strip() == "":
            # шукаємо вільне ім'я неназванийN.py
            n = 1
            while True:
                candidate = os.path.join(self.ddirectory, f"неназваний{n}.py")
                if not os.path.exists(candidate):
                    default_name = f"неназваний{n}.py"
                    break
                n += 1
        else:
            default_name = os.path.basename(self.txtDocument.filename)
    
        dlg = drFileDialog.SimpleSaveDialog(
            self,
            "Зберегти файл як ...",
            default_dir=self.ddirectory,
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            file_name = os.path.basename(self.txtDocument.filename)
            if not file_name.endswith(".py"):
                self.txtDocument.filename = self.txtDocument.filename + ".py"
            ...
            # Встановлення підсвічування синтаксису
    ...
    def OnSaveCopy(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                self.ShowMessage("Error copying file to: " + cfilename, "Помилка drPython")
        dlg.Destroy()
    ...
    def OnSavePrompt(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ctext = self.txtPrompt.GetText()

                cfile = open(pfilename, 'w', encoding='utf-8')
                cfile.write(ctext)
                cfile.close()
            except:
                self.ShowMessage("Error Writing: " + pfilename, "Помилка drPython")
            dlg.Destroy()
    ...
    def OnSetArgs(self, event):
        d = wx.TextEntryDialog(self, "Аргументи запуску:", "drPython - встановлення аргументів", self.lastprogargs)
    ...
    def OnToggleSourceBrowser(self, event):
        if self.SourceBrowser is None:
            target, i = self.mainpanel.GetTargetNotebookPage(self.prefs.sourcebrowserpanel, "Навігатор")
    ...
    def OnOpenWidgetInspector(self, evt):
        if self.prefs.debugmodus:
            ...
            try:
                ...
                self.ShowMessage("Inspect Tool not found\nPlease check your wxPython version!", "Помилка drPython")
                return
            ...
            self.ShowMessage("Please switch on debug modus in preferences menu", "Помилка drPython")
    ...
    def OnToolBar(self, event):
        try:
            ...
            self.ShowMessage("ToolBar Action Error", "Помилка drPython")
    ...
    def OpenFile(self, filename, OpenInNewTab, editrecentfiles = True, encoding="<Default Encoding>"):
        ...
        if not os.path.exists(filename):
            self.ShowMessage("Помилка при спробі відкрити: " + filename + " (File not exist)", "Помилка drPython")
            wx.EndBusyCursor()
            return
        try:
            cfile = open(filename, 'r', encoding='utf-8')
        except:
            self.ShowMessage("Помилка при спробі відкрити: " + filename, "Помилка drPython")
            wx.EndBusyCursor()
            return
        ...
        try:
            ...
            self.ShowMessage("Помилка при спробі відкрити: " + filename + "Processing failed", "Помилка drPython")
    ...
    def OpenOrSwitchToFile(self, filename, editRecentFiles=True, encoding="<Default Encoding>", force=False):
        ...
        if filename.lower() in alreadyopen:
            ...
            if not os.path.exists(filename):
                if not force:
                    self.ShowMessage("'" + filename + "' не існує.", "Файл не знайдено")
                else:
    ...
    def runcommand(self, command, statustext="Виконання", pagetext="Консоль", redin="", redout="", rederr=""):
        """
        Запускає Python-скрипт у модифікованому drPrompt.
        Параметр `command` має бути шляхом до .py-файлу (або командою, що містить шлях).
        """

        # Визначаємо шлях до файлу з команди
        # Припустимо, що команда має вигляд: "python /шлях/до/script.py" або просто "/шлях/до/script.py"
        script_path = None
        if command.strip().endswith('.py'):
            if ' ' in command:
                # Шукаємо останній аргумент, який закінчується на .py
                parts = command.split()
                for part in reversed(parts):
                    if part.endswith('.py') and os.path.isfile(part):
                        ...
                        break
                if not script_path:
                    # Спробуємо перший аргумент після "python"
            ...
            # Якщо це не .py — спробуємо знайти шлях у поточному документі
            if hasattr(self, 'txtDocument') and self.txtDocument.filename:
                script_path = self.txtDocument.filename

        if not script_path or not os.path.isfile(script_path):
            wx.MessageBox("Не вдається визначити шлях до Python-скрипта для запуску.", "Помилка", wx.OK | wx.ICON_ERROR)
            return

        # Оновлюємо інтерфейс: кнопки, статус
        if self.runBtn.IsShown():
            self.runBtn.Hide()
            self.stopBtn.Show()
            self.tool_msg.SetLabel("Виконується програма")
            self.tool_panel.Layout()

        # Шукаємо або створюємо сторінку консолі
        no_page = True
        if pagetext == ".unsaved.py":
            pagetext = "Консоль"
            statustext = "Виконується незбережений файл"
            no_page = False
        else:
            target_text = os.path.split(self.txtDocument.filename)[1]
            for i in range(self.promptnotebook.GetPageCount()):
                if self.promptnotebook.GetPageText(i) == target_text:
                    self.promptnotebook.SetSelection(i)
                    no_page = False
                    break
        ...
        # Показуємо панель промпта, якщо вона прихована
        ...
        # 🔑 Головний виклик: запуск через новий DrPrompt
        # 🔑 Визначаємо режим
        if "-i" in command and (".py" not in command or command.strip().endswith("-i")):
            # Це запуск інтерпретатора
            self.txtPrompt.StartInteractive()
        else:
            # Це запуск скрипта — витягуємо шлях до .py
            script_path = None
            parts = command.split()
            for part in reversed(parts):
                if part.endswith('.py') and os.path.isfile(part):
                    ...
                    break
            if not script_path:
                # Спроба з поточного документа
                if hasattr(self, 'txtDocument') and self.txtDocument.filename:
                    script_path = self.txtDocument.filename

            if script_path:
                self.txtPrompt.RunScript(script_path)
            else:
                wx.MessageBox("Не вдається знайти .py-файл для запуску.", "Помилка", wx.OK | wx.ICON_ERROR)
                self.run_state = False
                return
        ...
        try:
            if self.prefs.backupfileonsave and not IsSaveAs:
                try:
                    ...
                    self.ShowMessage("Error Backing up to: " + self.txtDocumentArray[docPos].filename + ".bak", "Помилка drPython")
            ...
            ctext = self.txtDocumentArray[docPos].GetText()
            ...
            if self.PLATFORM_IS_GTK:
              if (not os.access(self.txtDocumentArray[docPos].filename, os.W_OK)) and \
                  ...
                  #self.ShowMessage("Error Writing: " + self.txtDocumentArray[docPos].filename, "Помилка drPython")
                  if answer == wx.YES:
            ...
            self.ShowMessage("Помилка запису: " + self.txtDocumentArray[docPos].filename, "Помилка drPython")
            return False
    ...
    def setDocumentTo(self, number, ignoreold = 0):
        ...
        if not self.txtDocument.filename:
            title = "drPython - Без назви " + str(self.txtDocument.untitlednumber)
        else:
    ...
    def setPromptTo(self, number):
        ...
        if self.txtPromptArray[self.promptPosition].pid != -1:
            if self.txtPrompt.pythonintepreter:
                self.SetStatusText("Працює інтерпретатор Python", 2)
            else:
                self.SetStatusText(("Виконується " + os.path.split(self.txtDocument.filename)[1]), 2)
        else:
            self.SetStatusText("", 2)
    ...
    def setupdocumentsmenu(self):
        ...
        self.tabnavmenu.Append(self.ID_NEXT_DOCUMENT, "Наступна вкладка")
        self.tabnavmenu.Append(self.ID_PREVIOUS_DOCUMENT, "Попередня вкладка")
        self.tabnavmenu.Append(self.ID_FIRST_DOCUMENT, "Перша вкладка")
        self.tabnavmenu.Append(self.ID_LAST_DOCUMENT, "Остання вкладка")
        self.documentsmenu.AppendSubMenu(self.tabnavmenu, "Перехід")
        self.documentsmenu.AppendSeparator()
        self.documentsmenu.Append(self.ID_SAVE_ALL, "Зберегти вміст всіх вкладок")
        self.documentsmenu.AppendSeparator()
        self.documentsmenu.Append(self.ID_CLOSE_ALL, "Закрити всі вкладки")
        self.documentsmenu.Append(self.ID_CLOSE_ALL_OTHER_DOCUMENTS, "Закрити всі інші вкладки")
        self.documentsmenu.AppendSeparator()
    ...
    def UpdateMenuAndToolbar(self):
        isrunning = self.run_state #self.txtPrompt.pid != -1
        ...
        if not isrunning:
            ...
            self.programmenu.Enable(self.ID_RUN, True) #thereisafile)
        else:
...
#                    document.SourceBrowser = None
        ...
        if self.prefs.defaultidemode == "Класичний":
            self.toolbar.Show(True)
            self.tool_panel.Show(False)
        else:
            self.toolbar.Show(False)
    ...
    def ViewInPanel(self, event):
        ...
        target, i = self.mainpanel.GetTargetNotebookPage(self.viewinpaneltarget, "Додаткова панель")
        if docnumber < 0:
    ...
    def ViewInPanelMenu(self, event):
        ...
        x = 0
        l = len(self.txtDocumentArray)
        docMenu.Append(self.ID_VIEW_IN_PANEL_BASE-1, "Поточний файл")
        ...
        if l > 10:
            ...
            while y < yl:
                ...
                while x < yl:
                    if self.txtDocumentArray[x].filename:
                        ...
                        docSubMenus[a].Append(self.ID_VIEW_IN_PANEL_BASE+x, "Без назви " + str(self.txtDocumentArray[x].untitlednumber))
            ...
            while x < l:
                if self.txtDocumentArray[x].filename:
                    ...
                    docMenu.Append(self.ID_VIEW_IN_PANEL_BASE+x, "Без назви " + str(self.txtDocumentArray[x].untitlednumber))
                self.Bind(wx.EVT_MENU, self.ViewInPanel, id=self.ID_VIEW_IN_PANEL_BASE+x)
                x = x + 1
    ...
    def WriteRecentFiles(self):
        ...
        try:
            ...
            self.ShowMessage("Помилка запису: " + filename, "Помилка списку нещодавніх файлів")

    #Initialize menus for Advanced mode (more items)
    def CreateMenus(self):
        self.filemenu = drMenu(self)
        self.filemenu.AAppend(self.ID_NEW, "Новий", False, 0)
        self.filemenu.AAppend(self.ID_OPEN, "Відкрити", True, 0)
        self.recentmenu = self.CreateRecentFileMenu()
        self.filemenu.AppendSubMenu(self.recentmenu, "Відкрити нещодавні файли")
        self.filemenu.AppendSeparator()
        self.bookmarksmenu = drBookmarksMenu(self)
        self.filemenu.AppendSubMenu(self.bookmarksmenu, "Закладки")        
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_RELOAD, "Перезавантажити файл", False, 0)
        self.filemenu.Append(self.ID_RESTORE_FROM_BACKUP, "Відновити з резервної копії")
        self.filemenu.AAppend(self.ID_OPEN_IMPORTED_MODULE, "Відкрити імпортований модуль", True)
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_CLOSE, "Закрити", False, 0)
        self.filemenu.AppendSeparator()        
        self.filemenu.Append(self.ID_CLEAR_RECENT, "Спорожнити список нещодавніх файлів")      
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_SAVE, "Зберегти", False, 0)
        self.filemenu.AAppend(self.ID_SAVE_AS, "Зберегти як", True, 5)
        self.filemenu.AAppend(self.ID_SAVE_COPY, "Зберегти як копію", True, 2)
        self.filemenu.AAppend(self.ID_SAVE_PROMPT, "Зберегти вміст консолі до файлу", True)
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_PRINT_SETUP, "Налаштування друку", True)
        self.filemenu.AAppend(self.ID_PRINT, "Друк файлу", True, 0)
        self.filemenu.AAppend(self.ID_PRINTPROMPT, "Друк вмісту консолі", True)
        self.filemenu.AppendSeparator()
        self.filemenu.AAppend(self.ID_EXIT, "Вихід", False, 1)

        self.commentmenu = drMenu(self)
        self.commentmenu.Append(self.ID_COMMENT_REGION, "Коментувати")
        self.commentmenu.Append(self.ID_UNCOMMENT_REGION, "Розкоментувати")

        self.whitespacemenu = drMenu(self)
        self.whitespacemenu.AAppend(self.ID_INDENT_REGION, "Збільшити відступ", False, 0)
        self.whitespacemenu.AAppend(self.ID_DEDENT_REGION, "Зменшити відступ", False, 0)
        self.whitespacemenu.AppendSeparator()
        self.whitespacemenu.Append(self.ID_CHECK_INDENTATION, "Визначити тип відступів...")
        self.whitespacemenu.Append(self.ID_CLEAN_UP_TABS, "Встановити відступи табуляціями...")
        self.whitespacemenu.Append(self.ID_CLEAN_UP_SPACES, "Встановити відступи пропусками...")
        self.whitespacemenu.AppendSeparator()
        self.whitespacemenu.Append(self.ID_UNIXMODE, "Закінчення рядків як Unix/Linux (\"\\n\')")
        self.whitespacemenu.Append(self.ID_WINMODE, "Закінчення рядків як DOS/Windows (\"\\r\\n\')")
        self.whitespacemenu.Append(self.ID_MACMODE, "Закінчення рядків як Mac OS (\"\\r\')")

        self.casemenu = drMenu(self)
        self.casemenu.AAppend(self.ID_UPPERCASE, "У верхній регістр", False, 0)
        self.casemenu.AAppend(self.ID_LOWERCASE, "У нижній регістр", False, 0)

        self.editmenu = drMenu(self)

        self.editmenu.AAppend(self.ID_UNDO, "Скасувати", False, 0)
        self.editmenu.AAppend(self.ID_REDO, "Повернути", False, 1)
        self.editmenu.AppendSeparator()

        #Order changed by drpython
        ##for keyboard macro (Keyboardmacro) recording; #ugly hack franz # removed again

        self.editmenu.AAppend(self.ID_CUT, "Вирізати", False, 0)
        self.editmenu.AAppend(self.ID_COPY, "Копіювати", False, 0)
        self.editmenu.AAppend(self.ID_PASTE, "Вставити", False, 0)
        ##end patch keyboard macro recording #end ugly hack franz # removed again

        self.editmenu.AAppend(self.ID_DELETE, "Видалити", False, 0)

        self.editmenu.AppendSeparator()
        self.editmenu.AAppend(self.ID_SELECT_ALL, "Обрати все", False, 0)
        self.editmenu.AppendSeparator()
        self.editmenu.AAppend(self.ID_INSERT_SEPARATOR, "Вставити розділювач в код", True)
        self.editmenu.AAppend(self.ID_INSERT_REGEX, "Вставити регулярний вираз", True)
        self.editmenu.AppendSeparator()
        self.editmenu.Append(self.ID_FIND_AND_COMPLETE, "Автодоповнення")
        self.editmenu.AppendSeparator()
        self.editmenu.AppendSubMenu(self.commentmenu, "Коментарі")
        self.editmenu.AppendSubMenu(self.whitespacemenu, "Відступи")
        self.editmenu.AppendSubMenu(self.casemenu, "Перетворення регістру")

        self.searchmenu = drMenu(self)
        self.searchmenu.AAppend(self.ID_FIND, "Пошук", True, 0)
        self.searchmenu.AAppend(self.ID_FIND_NEXT, "Знайти наступне", False, 5)
        self.searchmenu.AAppend(self.ID_FIND_PREVIOUS, "Знайти попереднє", False, 5)
        self.searchmenu.AAppend(self.ID_REPLACE, "Замінити", True, 0)

        self.foldmenu = drMenu(self)
        self.foldmenu.AAppend(self.ID_TOGGLE_FOLD, "Згорнути/розгорнути", False, 0)
        self.foldmenu.AAppend(self.ID_FOLD_ALL, "Згорнути все", False, 0)
        self.foldmenu.AAppend(self.ID_EXPAND_ALL, "Розгорнути все", False, 0)
        ...
        self.viewinpanelmenu.Append(self.ID_VIEW_IN_LEFT_PANEL, "Показати у лівій панелі")
        self.viewinpanelmenu.Append(self.ID_VIEW_IN_RIGHT_PANEL, "Показати у правій панелі")
        self.viewinpanelmenu.Append(self.ID_VIEW_IN_TOP_PANEL, "Показати у верхній панелі")

        self.gotomenu = drMenu(self)
        self.gotomenu.AAppend(self.ID_GOTO_BLOCK_START, "Перейти до початку блоку", False, -1, "Початок блоку")
        self.gotomenu.AAppend(self.ID_GOTO_BLOCK_END, "Перейти до кінця блоку", False, -1, "Кінець блоку")
        self.gotomenu.AAppend(self.ID_GOTO_CLASS_START, "Перейти до початку визначення класу", False, -1, "Початок класу")
        self.gotomenu.AAppend(self.ID_GOTO_CLASS_END, "Перейти до кінця визначення класу", False, -1, "Кінець класу")
        self.gotomenu.AAppend(self.ID_GOTO_DEF_START, "Перейти до початку функції", False, -1, "Початок функції")
        self.gotomenu.AAppend(self.ID_GOTO_DEF_END, "Перейти до кінця функції", False, -1, "Кінець функції")

        self.viewmenu = drMenu(self)
        self.viewmenu.AAppend(self.ID_GOTO, "Перейти до рядка", True, 0)
        self.viewmenu.AppendSubMenu(self.gotomenu, "Перейти")
        self.viewmenu.AppendSeparator()
        self.viewmenu.AAppend(self.ID_ZOOM_IN, "Збільшити символи", False, 5)
        self.viewmenu.AAppend(self.ID_ZOOM_OUT, "Зменшити символи", False, 5)
        self.viewmenu.AppendSeparator()
        self.viewmenu.AppendSubMenu(self.foldmenu, "Згортання")
        self.viewmenu.AppendSeparator()
        self.viewmenu.AppendSubMenu(self.viewinpanelmenu, "Показати у панелі")
        self.viewmenu.AppendSeparator()
        self.viewmenu.AppendSubMenu(self.highlightmenu, "Виділення синтаксису")
        self.viewmenu.AppendSeparator()
        self.viewmenu.Append(self.ID_TOGGLE_SOURCEBROWSER, "Показати/сховати Навігатор")
        self.viewmenu.AAppend(self.ID_SOURCEBROWSER_GOTO, "Перехід за структурою", True)
        self.viewmenu.AppendSeparator()
        #fix bug someone refered in forum limodou 2004/04/20
        self.viewmenu.AAppend(self.ID_TOGGLE_VIEWWHITESPACE, "Показати/сховати індикатори пропусків", False, 12)
        #end limodou
        self.viewmenu.Append(self.ID_TOGGLE_PROMPT, "Показати/сховати консоль")

        self.programmenu = drMenu(self)
        self.programmenu.AAppend(self.ID_CHECK_SYNTAX, "Перевірити синтаксис", False, 0)
        self.programmenu.AppendSeparator()
        self.programmenu.AAppend(self.ID_RUN, "Виконати", False, 0)
        self.programmenu.AAppend(self.ID_SET_ARGS, "Встановити аргументи", True)
        self.programmenu.AAppend(self.ID_PYTHON, "Python", False, -1, "Відкрити інтерпретатор Python")
        self.programmenu.AAppend(self.ID_END, "Зупинити", False, 0)
        self.programmenu.Append(self.ID_CLOSE_PROMPT, "Закрити консоль")
        # Feb 17 - adding PyDoc menu items
        self.programmenu.AppendSeparator()
        self.programmenu.Append(self.ID_PYDOC_CURR, self.getmenulabel("Pydoc поточного файлу"))
        self.programmenu.Append(self.ID_PYDOC_ALL, self.getmenulabel("Pydoc всіх відкритих файлів"))
        self.programmenu.Append(self.ID_VIEW_PYDOC, self.getmenulabel("Огляд PyDoc..."))
        # End Pydoc changes
        ...
        self.optionsmenu.AAppend(self.ID_PREFS, "Налаштування", True, 0)
        self.optionsmenu.AAppend(self.ID_SHORTCUTS, "Налаштувати сполучення клавіш", True, 0)
        self.optionsmenu.AAppend(self.ID_POPUP, "Налаштувати вигулькнe меню", True)
        self.optionsmenu.AAppend(self.ID_CUSTOMIZE_TOOLBAR, "Налаштувати панель засобів", True)
        self.optionsmenu.AppendSeparator()
        self.optionsmenu.AAppend(self.ID_EDIT_BOOKMARKS, "Змінити закладки", True, 0)
        #self.optionsmenu.AAppend(self.ID_EDIT_SCRIPT_MENU, "Змінити меню скриптів", True)
        self.optionsmenu.AppendSeparator()
        self.optionsmenu.AppendSubMenu(self.pluginsconfiguremenu, "Додавання/видалення доповнень")
        self.optionsmenu.AppendSubMenu(self.pluginsprefsmenu, "Налаштування доповнень")
        self.optionsmenu.AppendSubMenu(self.pluginsindexmenu, "Завантажити доповнення з індексу")
        self.optionsmenu.Append(self.ID_EDIT_PLUGIN, "Редагувати код доповнення")
        self.optionsmenu.AppendSeparator()
        self.optionsmenu.AppendSubMenu(self.drscriptmenu, "Скрипти")
        self.pluginshelpmenu = drPluginHelpMenu(self)
        self.pluginsaboutmenu = drPluginAboutMenu(self)

        self.helpmenu = drMenu(self)
        self.helpmenu.Append(self.ID_ABOUT, "Про drPython...")
        self.helpmenu.AppendSubMenu(self.pluginsaboutmenu, "Про доповнення")
        self.helpmenu.AppendSeparator()
        self.helpmenu.AAppend(self.ID_HELP, "Help", True, 0, "Допомога по drPython...")
        self.helpmenu.AppendSubMenu(self.pluginshelpmenu, "Допомога по доповненнях")
        self.helpmenu.AppendSeparator()
        self.helpmenu.AAppend(self.ID_PYTHON_DOCS, "Перекляд документації Python", True)
        self.helpmenu.AAppend(self.ID_WXWIDGETS_DOCS, "Перегляд документації WxWidgets", True)
        self.helpmenu.AAppend(self.ID_REHOWTO_DOCS, "Посібник по регулярних виразах", True)

        if self.prefs.debugmodus:
            self.helpmenu.AppendSeparator()
            self.helpmenu.Append(self.ID_INSPECT_TOOL, self.getmenulabel("Інспектор"))

        self.menuBar = wx.MenuBar()
        menuBarNames = ["Файл", "Зміни", "Пошук", "Перегляд", "Програма", "Вкладки", "Налаштування", "Допомога"]
        ...
        self.menuBar.Append(self.documentsmenu, menuBarNames[5])
        self.menuBar.Append(self.optionsmenu, menuBarNames[6])
        #self.menuBar.Append(self.drscriptmenu, menuBarNames[7])
        self.menuBar.Append(self.helpmenu, menuBarNames[7])
    ...
    def SetToolbar(self):
        ...
        try:
            ...
            self.ShowMessage("Помилка завантаження списку панелі інструментів", "Помилка drPython")
    ...
    def promptSaveAll(self):
        ...
        while x < len(self.txtDocumentArray):
            if self.txtDocumentArray[x].GetModify():
                if self.Ask("Бажаєте зберегти #%s#?" % self.txtDocumentArray[x].GetFilename(), "drPython"):
                    self.setDocumentTo(x)
                    self.OnSave(None)
            x += 1
...
class DrApp(wx.App):
    ...
    def OnIdle(self, event):
        """Перевіряє наявність тимчасового файлу openfile.tmp і відкриває вказаний документ."""
        self.idlecnt += 1
        if self.idlecnt == 2:
            self.idlecnt = 0
            openfiletmp = os.path.join(os.path.expanduser('~'), "openfile.tmp")
            if os.path.isfile(openfiletmp):
                with open(openfiletmp, 'r', encoding='utf-8') as f:
                    filename = f.read().strip()
                os.remove(openfiletmp)
                if filename:
                    self.frame.OpenOrSwitchToFile(filename, force=True)

    def is_already_running(self, process_name="drpython.py"):
        """Перевіряє, чи вже запущений інший процес з такою ж назвою."""
        ...
        for proc in psutil.process_iter(["pid", "cmdline"]):
            try:
                ...
                # Перевіряємо, чи це той самий застосунок
                if process_name in cmd or "drpython" in cmd.lower():
                    return True
    ...
    def OnInit(self):
        """Ініціалізація застосунку без wx.SingleInstanceChecker, з кросплатформною перевіркою psutil."""
        ...
        if get_option("--s"):
            try:
                if self.is_already_running():
                    # Якщо вже є запущений DrPython — передаємо файл і завершуємось
                    if len(sys.argv) > 1:
                        openfiletmp = os.path.join(os.path.expanduser('~'), "openfile.tmp")
                        with open(openfiletmp, 'w', encoding='utf-8') as f:
                            f.write(sys.argv[-1])
                    wx.MessageBox(
                        "DrPython уже запущено.\n"
                        "Файл буде відкрито у наявному вікні.",
                        "drPython - повідомлення",
                        wx.ICON_INFORMATION
                    )
                    sys.exit(0)
            except Exception as e:
                print(f"Помилка перевірки процесів: {e}")
        ...
        self.frame = DrFrame(None, 101, "DrPython - Без назви 1")
    ...
    def OnActivate(self, event):
        """Обробка активації/деактивації вікна."""
        return
        if event.GetActive():
