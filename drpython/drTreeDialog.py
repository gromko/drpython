...
class drTree(wx.TreeCtrl):
    def __init__(self, parent, id, targetbitmap, stylestring, point, size, style):
        ...
        self.images = [wx.Bitmap(wx.Image(os.path.join(self.grandparent.bitmapdirectory, "16/folder.png"), wx.BITMAP_TYPE_PNG)), \
...
class drTreeDialog(wx.Dialog):
    ...
    def __init__(self, parent, title, rootstring, targetfile, targetstyle, dialogsizefile, bitmapfile, BuildTreeFromString, WriteBranch):
        ...
        self.btnAddFolder = wx.Button(self, self.ID_ADD_FOLDER, "Додати збірку")
        self.btnRemove = wx.Button(self, self.ID_REMOVE, "Видалити")
        self.btnSort = wx.Button(self, self.ID_SORT, "Впорядкувати збірки")
    ...
    def initialize(self):
        if os.path.exists(self.targetfile):
            try:
                ...
                drScrolledMessageDialog.ShowMessage(self.parent, "Помилка файлу.\n'" + self.targetfile + "'\n", "Помилка")
    ...
    def OnbtnAddFolder(self, event):
        ...
        if self.datatree.GetItemText(sel)[0] == '>':
            d = wx.TextEntryDialog(self, "Введіть назву збірки:", "Додати збірку", "")
    ...
    def OnbtnClose(self, event):
        if self.datatree.GetModified():
            answer = wx.MessageBox("Зберегти зміни?", self.title, wx.YES_NO | wx.CANCEL | wx.ICON_QUESTION)
            if answer == wx.YES:
    ...
    def OnbtnRemove(self, event):
        ...
        if sel == self.datatree.GetRootItem():
            drScrolledMessageDialog.ShowMessage(self, "Ви не можете видалити кореневий об'єкт.\n", "Помилка")
            return
        t = self.datatree.GetItemText(sel)
        message = "Буде видалено: " + t
        if self.datatree.GetItemText(sel)[0] == '>':
            message = "Буде видалено всі об'єкти у збірці: " + t + "!"
        answer = wx.MessageBox((message + "\nВи погоджуєтесь?"), "drPython", wx.YES_NO | wx.ICON_QUESTION)
        if answer == wx.NO:
            return
        try:
    ...
    def OnbtnSave(self, event):
        try:
            ...
            f = open(self.targetfile, 'w',encoding="UTF-8")
        ...
        if self.parent.prefs.enablefeedback:
            drScrolledMessageDialog.ShowMessage(self, ("Успішно збережено:\n"  + self.targetfile), "Збережено")
    ...
    def GetTreeCtrlItems(self, tree, branch, tablevel, skipfolders=True):
        """
        Рекурсивно отримує елементи дерева та додає їх до списку curentries.
        
        :param tree: Об'єкт wx.TreeCtrl.
        :param branch: Поточний вузол дерева.
        :param tablevel: Рівень вкладеності (для відступів).
        :param skipfolders: Якщо True, папки не додаються до списку.
        """
        t = tree.GetItemText(branch)
        isfolder = (t[0] == '>')
    
        # Якщо це не папка, додаємо до списку
        if not isfolder:
            self.curentries.append(t)
    
        # Якщо це папка і skipfolders=False, додаємо її до списку
        if isfolder and not skipfolders:
            self.curentries.append(t)
    
        # Рекурсивно обробляємо дочірні елементи
        if isfolder:
            child, cookie = tree.GetFirstChild(branch)  # Отримуємо перший дочірній елемент
            while child.IsOk():  # Перевіряємо, чи елемент є дійсним
                self.GetTreeCtrlItems(tree, child, tablevel + 1, skipfolders)  # Рекурсивний виклик
                child, cookie = tree.GetNextChild(branch, cookie)  # Отримуємо наступний дочірній елемент
    
    def GetCurrentItems(self, skipfolders=True):
        """
        Отримує всі елементи дерева та зберігає їх у списку curentries.
        
        :param skipfolders: Якщо True, папки не додаються до списку.
        """
        self.curentries = list()  # Очищуємо список перед початком
        root = self.datatree.GetRootItem()  # Отримуємо кореневий елемент
        if root.IsOk():  # Перевіряємо, чи кореневий елемент дійсний
            self.GetTreeCtrlItems(self.datatree, root, 0, skipfolders)  # Починаємо рекурсивний обхід
