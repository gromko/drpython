...
class drFileDialog(wx.Dialog):
    ...
    def OnAddBookmark(self, event):
        try:
            ...
            f = open(bookfile, 'r',encoding="UTF-8")
            text = f.read()
            f.close()
            ...
            f = open(bookfile, 'w',encoding="UTF-8")
            f.write(text)
            f.close()
...
class SimpleSaveDialog(wx.Dialog):
    def __init__(self, parent, title="Зберегти файл як ...", default_dir="", default_name="newfile.py"):
        super(SimpleSaveDialog, self).__init__(parent, title=title, size=(450, 150))

        self.selected_dir = default_dir if default_dir else os.path.expanduser("~")

        panel = wx.Panel(self)
        vbox = wx.BoxSizer(wx.VERTICAL)

        # Поле для імені файлу
        hbox1 = wx.BoxSizer(wx.HORIZONTAL)
        lbl_name = wx.StaticText(panel, label="Ім'я файлу:")
        self.txt_name = wx.TextCtrl(panel, value=default_name)
        hbox1.Add(lbl_name, 0, wx.ALL | wx.CENTER, 5)
        hbox1.Add(self.txt_name, 1, wx.ALL | wx.EXPAND, 5)
        vbox.Add(hbox1, 0, wx.EXPAND)

        # Поле каталогу + кнопка
        hbox2 = wx.BoxSizer(wx.HORIZONTAL)
        lbl_dir = wx.StaticText(panel, label="Каталог:")
        self.txt_dir = wx.TextCtrl(panel, value=self.selected_dir)
        btn_browse = wx.Button(panel, label="...")
        btn_browse.Bind(wx.EVT_BUTTON, self.on_browse)
        hbox2.Add(lbl_dir, 0, wx.ALL | wx.CENTER, 5)
        hbox2.Add(self.txt_dir, 1, wx.ALL | wx.EXPAND, 5)
        hbox2.Add(btn_browse, 0, wx.ALL, 5)
        vbox.Add(hbox2, 0, wx.EXPAND)

        # Кнопки внизу
        hbox3 = wx.BoxSizer(wx.HORIZONTAL)
        btn_save = wx.Button(panel, wx.ID_OK, "Зберегти")
        btn_cancel = wx.Button(panel, wx.ID_CANCEL, "Скасувати")
        hbox3.AddStretchSpacer(1)
        hbox3.Add(btn_save, 0, wx.ALL, 5)
        hbox3.Add(btn_cancel, 0, wx.ALL, 5)
        vbox.Add(hbox3, 0, wx.EXPAND)

        panel.SetSizer(vbox)

    def on_browse(self, event):
        dlg = wx.DirDialog(self, "Оберіть каталог", self.selected_dir)
        if dlg.ShowModal() == wx.ID_OK:
            self.selected_dir = dlg.GetPath()
            self.txt_dir.SetValue(self.selected_dir)
        dlg.Destroy()

    # ==== Додані методи для сумісності ====
    def SetDirectory(self, directory):
        """Встановити каталог"""
        self.selected_dir = directory
        self.txt_dir.SetValue(directory)

    def SetFilename(self, filename):
        """Встановити ім'я файлу"""
        self.txt_name.SetValue(filename)

    def GetPath(self):
        """Повернути повний шлях (сумісно зі старим кодом)"""
        return os.path.join(self.txt_dir.GetValue(), self.txt_name.GetValue())

    # Для нового коду можна залишити коротку версію
    def get_path(self):
        return self.GetPath()
