...
class DrText(drSTC.DrStyledTextControl):
    ...
    def GetFilename(self):
        ...
        return "Без назви " + str(self.untitlednumber)
        ...
        if not self.IsSplitView:
            ...
            if (modify != self.modified) or (event is None):
                ...
                if not self.filename:
                    title = "drPython - Без назви " + str(self.untitlednumber)
                    pagetext = "Без назви " + str(self.untitlednumber)
                else:
    ...
    def OnPositionChanged(self, event):
        ...
        if self.GetOvertype():
            ...
            ovrstring = "INS"
        statustext = "рядок: %(line)s, позиція: %(col)s   %(mode)s   %(ovrstring)s   %(ind)s" \
    ...
    def SetupLineNumbersMargin(self):
        if self.grandparent.prefs.docshowlinenumbers:
            ...
            textwidth = self.TextWidth(wx.stc.STC_STYLE_LINENUMBER, lstring)            
