...
def ParseImportStatement(matches):
    targetarray = list(map(lambda x: x.strip().split(), matches))
    results = []
...
class drOpenImportedModuleDialog(drSingleChoiceDialog):
    def __init__(self, parent, modulelist, point=wx.DefaultPosition, size=(350, 300)):
        drSingleChoiceDialog.__init__(self, parent, "Відкрити імпортований модуль", modulelist, point, size)
