...
def OnPopUp(stc, event):
        ...
        if not stc.grandparent.popupmenulist:
            stc.grandparent.popupmenulist = ["Скасувати", "Повернути", "<Separator>", "Вирізати", "Копіювати", "Вставити", "Видалити", "<Separator>", "Обрати все"]
...
def OnPopUpMenu(stc, event):
        eid = event.GetId()
        ulabel = stc.PopUpMenu.GetLabel(eid)
        label = ukr_popupmenulist[ulabel]
