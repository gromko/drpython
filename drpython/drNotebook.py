...
class drNotebook(wx.Notebook):
    def __init__(self, parent, id, images, closefunction):
        ...
        if images:
            ...
            list(map(self.imagelist.Add, self.images))
...
class drDocNotebook(drNotebook):
    ...
    def OnPopUp(self, event):
        ...
        tabmenu = wx.Menu()
        tabmenu.Append(self.grandparent.ID_CLOSE, "Закрити")
        tabmenu.Append(self.grandparent.ID_CLOSE_ALL, "Закрити всі вкладки")
        tabmenu.Append(self.grandparent.ID_CLOSE_ALL_OTHER_DOCUMENTS, "Закрити всі інші вкладки")
        tabmenu.AppendSeparator()
        tabmenu.Append(self.grandparent.ID_NEXT_DOCUMENT, "Наступна вкладка")
        tabmenu.Append(self.grandparent.ID_PREVIOUS_DOCUMENT, "Попередня вкладка")
        tabmenu.Append(self.grandparent.ID_FIRST_DOCUMENT, "Перша вкладка")
        tabmenu.Append(self.grandparent.ID_LAST_DOCUMENT, "Остання вкладка")
        tabmenu.AppendSeparator()
        tabmenu.Append(self.grandparent.ID_RELOAD, "Перезавантажити файл")
        tabmenu.Append(self.grandparent.ID_RESTORE_FROM_BACKUP, "Відновити з резервної копії")
        tabmenu.AppendSeparator()
        tabmenu.Append(self.grandparent.ID_SAVE, "Зберегти")
        tabmenu.Append(self.grandparent.ID_SAVE_AS, "Зберегти як...")
...
class drPromptNotebook(drNotebook):
    ...
    def OnPopUp(self, event):
        ...
        tabmenu = wx.Menu()
        tabmenu.Append(self.grandparent.ID_CLOSE_PROMPT, "Закрити консоль")
        tabmenu.Append(self.grandparent.ID_CLEAR_PROMPT, "Очистити")
        tabmenu.AppendSeparator()
        tabmenu.Append(self.grandparent.ID_PYTHON, "Python")
        tabmenu.Append(self.grandparent.ID_RUN, "Виконати поточний файл")
        tabmenu.Append(self.grandparent.ID_END, "Зупинити")
...
class drSidePanelNotebook(drNotebook):
    ...
    def OnPopUp(self, event):
        ...
        tabmenu = wx.Menu()
        tabmenu.Append(self.ID_CLOSE, "Закрити панель")
...
class drSplitTextPanel(wx.Panel):
    def __init__(self, parent, grandparent, targetstc, position, index):
        ...
        self.label = wx.TextCtrl(self, -1, " Файл: " + targetstc.GetFilenameTitle(), size=(150, -1), style=wx.TE_READONLY)

        close_bitmap = wx.ArtProvider.GetBitmap(wx.ART_CLOSE, wx.ART_BUTTON)
        self.btnClose = wx.BitmapButton(self, id=ID_CLOSE, bitmap=close_bitmap, pos=(20, 20))
        #self.btnClose = wx.Button(self, ID_CLOSE, "Закрити")
       
        self.topSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.theSizer = wx.BoxSizer(wx.VERTICAL)

        self.topSizer.Add(self.label, 1, wx.EXPAND)
        self.topSizer.Add(self.btnClose, 0, wx.SHAPED|wx.EXPAND)
