...
def GetShortcuts(filename, frame):
    try:
        ...
        drScrolledMessageDialog.ShowMessage(frame, "Помилка файлу: '" + filename + "'.", "ERROR")
        return ""
...
class drShortcutPanel(wx.Panel):
    ...
    def __init__(self, parent, id, wxSize):
        ...
        self.btnGetKey = wx.Button(self, self.ID_GET_KEY, "Отримати")

        self.parent = parent

        self.theSizer.Add(wx.StaticText(self, -1, ""), 1, wx.ALIGN_LEFT | wx.SHAPED)
        self.theSizer.Add(wx.StaticText(self, -1, "Символ:"), 1, wx.ALIGN_LEFT | wx.SHAPED)
        self.theSizer.Add(self.txtKeyChar, 1, wx.ALIGN_LEFT | wx.SHAPED)

        self.theSizer.Add(wx.StaticText(self, -1, ""), 1, wx.ALIGN_LEFT | wx.SHAPED)
        self.theSizer.Add(wx.StaticText(self, -1, "Код клавіші:"), 1, wx.ALIGN_LEFT | wx.SHAPED)
    ...
    def OnbtnGetKey(self, event):
        ...
        if alreadytaken > -1:
            ...
            drScrolledMessageDialog.ShowMessage(self, "Сполучення клавіш '" + newv + \
            "'\nвже використано для '" + takenby + "'.\nDrPython вимушений ігнорувати ваш запит.",\
            "Це сполучення вже використовується")
            return
    ...
    def SetShortcut(self, shortcut, shortcutIndex, listIndex):
        try:
            ...
            drScrolledMessageDialog.ShowMessage(self.parent, "Помилка вибру сполучення клаіш", "Помилка DrPython")
            return
...
class drShortcutsDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, ("Налаштування сполучень клавіш"), wx.DefaultPosition, (-1, -1), wx.DEFAULT_DIALOG_STYLE )
        ...
        drscripshortcutssorted = list(zip(*sorted(zip(self.parent.DrScriptShortcutNames, self.parent.DrScriptShortcuts), key=lambda x: x[0])))
        ...
        self.btnGetIgnoreKeys = wx.Button(self, self.ID_IGNORE, "Ігнорувати...")
        ...
        self.listSizer.Add(wx.StaticText(self, -1, "Список: "), 0, wx.ALIGN_CENTER | wx.SHAPED)
        self.listSizer.Add(self.cboList, 0, wx.ALIGN_CENTER | wx.SHAPED)

        self.ignoreSizer.Add(wx.StaticText(self, -1, "Ігнорувати: "), 0, wx.ALIGN_CENTER | wx.SHAPED)
        ...
        self.theSizer.Add(wx.StaticText(self, -1, "   "), 0, wx.ALIGN_CENTER | wx.SHAPED)
        self.theSizer.Add(wx.StaticText(self, -1, "Сполучення:"), 0, wx.ALIGN_CENTER | wx.SHAPED)

        self.theSizer.Add(wx.StaticText(self, -1, ""), 0, wx.ALIGN_CENTER | wx.SHAPED)
        self.theSizer.Add(self.boxShortcuts, 0, wx.SHAPED | wx.ALIGN_CENTER)
        self.theSizer.Add(wx.StaticText(self, -1, ""), 0, wx.ALIGN_CENTER | wx.SHAPED)
        self.theSizer.Add(self.pnlShortcut, 1,  wx.ALIGN_CENTER)
                ...
                if list:
                    ...
                    if needtomakefile:
                        try:
                            ...
                            drScrolledMessageDialog.ShowMessage(self, ("Помилка створення сполучень клавіш для розширення"), "Помилка сполучень клавіш для розширень")
                            return
    ...
    def OnbtnGetIgnoreKeys(self, event):
        ...
        if self.parent.PLATFORM_IS_WIN:
            addstring = '\n\n\n\n\n\n'
        d = wx.lib.dialogs.MultipleChoiceDialog(self, "Оберіть клавіші-модифікатори, які будуть ігноруватися:" + addstring,
            "Ігнорувати модифікатори", ["Control", "Meta", "Shift", "Alt"])
        answer = d.ShowModal()
        if answer == wx.ID_OK:
    ...
    def OnbtnReset(self, event):
        answer = wx.MessageBox("Це скине всі сполучення клавіш програми до типових.\n(Для застосування вам все одно потрібно натиснути Оновити та/або Зберегти)\nВи впевнені, що хочете це зробити?",
            "Скинути сполучення", wx.YES_NO | wx.ICON_QUESTION)
        if answer == wx.YES:
    ...
    def OnbtnSave(self, event):
        ...
        try:
            ...
            drScrolledMessageDialog.ShowMessage(self, ("Виникли проблеми щодо запису до:\n"  + shortcutsfile + "\nФайл має метафізичні проблеми, або у вас немає дозволу на запис.\nЩодо метафізичних проблем, зверніться до документації.\nЩодо проблем з дозволом, змініть дозволи на теку, щоб дозволити собі доступ на запис.\nDrPython відхилив ваш запит на збереження.\nСпробуйте ще раз, коли вирішите проблему."), "Помилка запису файлу")
            return
        ...
        if self.parent.prefs.enablefeedback:
            drScrolledMessageDialog.ShowMessage(self, ("Налаштування поточного екземпляру DrPython\nуспішно оновлено та збережено."), "Сполучення клавіш збережено")

        wx.MessageBox("Для оновлення сполучень клавіш для меню, перезапустіть DrPython", "Інформація")
    ...
    def OnbtnUpdate(self, event):
        ...
        if self.parent.prefs.enablefeedback:
            drScrolledMessageDialog.ShowMessage(self, ("Налаштування поточного екземпляру DrPython успішно оновлено.\Натисніть Зберегти, щоб зробити їх постійними."), "Сполучення клавіш оновлено")
