...
class drAboutContentPanel(wx.Panel):
    def __init__(self, parent, id, drframe):
        ...
        app = wx.lib.stattext.GenStaticText(self, -1, "Середовище програмування DrPython")
        app.SetBackgroundColour(wx.WHITE)

        app.SetFont(standardfont)

        author = wx.lib.stattext.GenStaticText(self, -1, "(c) 2003-2010, Daniel Pozmanter\nПеренесення на Python 3 та переклад - Г.Громко")
        author.SetBackgroundColour(wx.WHITE)

        author.SetFont(standardfont)

        credits = drStaticLink(self, 1, " Подяки ", os.path.join(drframe.programdirectory, "documentation/credits.html"), drframe)
...
class drLicensePanel(wx.Panel):
    def __init__(self, parent, id, drframe):
        ...
        try:
            ...
            drframe.ShowMessage("Неможливо завантажити текст ліцензії GPL!", "Помилка діалогового вікна 'Про програму'")
            return
...
class drAboutDialog(wx.Dialog):
    ...
    def __init__(self, parent, app_version):
        wx.Dialog.__init__(self, parent, -1, ("Про DrPython"), wx.DefaultPosition, (500, 400), wx.DEFAULT_DIALOG_STYLE | wx.MAXIMIZE_BOX | wx.RESIZE_BORDER)

        self.parent = parent

        self.notebook = wx.Notebook(self, -1, style=wx.CLIP_CHILDREN)

        self.notebook.AddPage(drAboutPanel(self.notebook, -1, parent), "Про програму")
        self.notebook.AddPage(drLicensePanel(self.notebook, -1, parent), "Ліцензійні умови")
        self.notebook.AddPage(drSystemPanel(self.notebook, -1), "Системна інформація")
