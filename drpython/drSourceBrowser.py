...
class drTree(wx.TreeCtrl):
    def __init__(self, parent, id, point, size, style, ancestor):
        ...
        # Ініціалізація ImageList для значків
        if self.grandparent.prefs.sourcebrowseruseimages == 1:
            ...
            self.images = [
                self.load_bitmap(os.path.join(self.grandparent.bitmapdirectory, "16/class.png")),
                self.load_bitmap(os.path.join(self.grandparent.bitmapdirectory, "16/def.png")),
                self.load_bitmap(os.path.join(self.grandparent.bitmapdirectory, "16/import.png")),
                self.load_bitmap(os.path.join(self.grandparent.bitmapdirectory, "16/transparent.png")),
                self.load_bitmap(os.path.join(self.grandparent.bitmapdirectory, "16/comment.png"))
            ]
        ...
        # Налаштування шрифту
        # 0 =face, 1= size, 2 = underline, 3 = bold, 4 =italic
        
        w= wx.Font(yarrr[1], wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, underline=False,faceName=yarrr[0])
    ...
    def load_bitmap(self, path):
        """Завантажує bitmap з файлу, якщо він існує."""
        if os.path.exists(path):
            return wx.Bitmap(wx.Image(path, wx.BITMAP_TYPE_PNG))
        else:
            print(f"Warning: Image file not found: {path}")
            return wx.Bitmap(16, 16)  # Повертаємо пустий bitmap, якщо файл не знайдено
    ...
    def OnItemActivated(self, event):
        ...
        try:
            ...
            drScrolledMessageDialog.ShowMessage(self.parent, "Помилка активації об'єкта", "Помилка Навігатора")
...
class drSourceBrowserPanel(wx.Panel):
    def __init__(self, parent, id, Position, Index):
        ...
        self.reinspect = re.compile(r'(^[ \t]*?class\s.*[(:])|(^[ \t]*?def\s.*[(:])|(^[ \t]*?import\s.*$)|(^[ \t]*?from\s.*$)|(^[ \t]*## .+)|(^\s*$)', re.MULTILINE | re.UNICODE)
        ...
        self.btnRefresh = wx.Button(self, 102, "Оновити")
        ...
        self.edSearch.SetToolTip("Шукати у структурі")
        self.eol = self.parent.txtDocument.GetEndOfLineCharacter()

        if not self.Browse():
            self.mixed = 1
            msg = "Цей документ має суміш пропусків і табуляцій. Сумісне використання табуляцій та пропусків  для відступів\n не дає змоги коректно відобразити структуру проєкту. Виправте це за допомогою Зміни > Відступи."
            drScrolledMessageDialog.ShowMessage(self, msg, "Результати перевірки відступів")
    ...
    def Browse(self):
        ...
        while match is not None:
            matchedtext = match.group()  # Отримуємо текст, який відповідає регулярному виразу
        
            # Пропустити порожні рядки
            if not matchedtext.strip():
                try:
                    match = next(matcher)
                except:
                    match = None
            ...
            # Обробка класів, функцій, імпортів тощо
            ...
            currentIndent = currentIndent + 1
            currentitem = self.classtree.AppendItem(Roots[currentRoot], matchedtext.lstrip())  # Видаляємо ведучі пропуски
            ...
            self.ItemsPos.append(len(self.targetText[:match.start()].encode('utf-8')))
            
            # Призначення значків для класів, функцій, імпортів тощо
            
                
            if matchedtext[0] == 'c':  # Класи
                try:
            ...
            elif matchedtext[0] == '#':  # Коментарі
                try:
                    fg, bg = convertStyleToColorArray(self.parent.prefs.PythonStyleDictionary[6])
                    self.classtree.SetItemTextColour(Roots[currentRoot], fg)
                    self.classtree.SetItemBackgroundColour(Roots[currentRoot], bg)
                except Exception as e:
                    print("Error Setting Comments Colour:", e)
                self.classtree.SetItemImage(Roots[currentRoot], 4, wx.TreeItemIcon_Normal)
                self.classtree.SetItemImage(Roots[currentRoot], 4, wx.TreeItemIcon_Expanded)            
            
            else:  # Імпорти та інше
